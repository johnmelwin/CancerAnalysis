...
#Build model to predict incidenceRate with PovertyEst, popEst2015, catfiveYearTrend, deathRate, medIncome
fit2 = lm(incidenceRate ~ PovertyEst + popEst2015 + deathRate + avgDeathsPerYear+medIncome, data=df)
summary(fit2)
...
# Add catfiveYearTrend, recentTrend, recTrend, avgAnnCount
fit = lm(incidenceRate ~ PovertyEst + popEst2015 + avgAnnCount +catfiveYearTrend + deathRate + avgDeathsPerYear+medIncome + recentTrend + recTrend, data=df)
summary(fit)
...
fit3 = lm(incidenceRate ~ . -Name -State- avgDeathsPerYear -catfiveYearTrend -countyCode - recTrend - recentTrend  -avgAnnCount, data = df)
summary(fit3)
...
fit4 = lm(incidenceRate ~ . -Name -State- avgDeathsPerYear -catfiveYearTrend -countyCode - recTrend - recentTrend  -avgAnnCount + popEst2015*medIncome, data = df)
summary(fit4)


## STEP 5: Checking the assumptions for fourth model-> fit4

# 1: linear relationships
plot(df$PovertyEst, fit4$residuals)
abline(0,0)
# No linear relationship

plot(df$medIncome, fit4$residuals)
abline(0,0)
# No linear relationship

plot(df$popEst2015, fit4$residuals)
abline(0,0)
# No major linear relationship

plot(df$fiveYearTrend, fit4$residuals)
abline(0,0)
# No linear relationship

plot(df$deathRate, fit4$residuals)
abline(0,0)
# No linear relationship
...
# After checking assumptions, model-4 passes all tests and explains maximum variability with the least and most important factors so model-4 it is selected.